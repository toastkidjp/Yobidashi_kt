...
class ContextMenuBuilder(
    ...
    operator fun invoke(): TextContextMenuBuilderScope.() -> Unit =
        { constructMenuContent() }

    private fun TextContextMenuBuilderScope.constructMenuContent() {
        if (viewModel.selectedText().isEmpty()) {
            ...
            item(key = "Ordered list", label = "Ordered list") {
                val newText = NumberedListHeadAdder().invoke(viewModel.selectedText())
                    ?: return@item close()
            ...
            item(key = "List", label = "To list") {
                val newText =
                    ListHeadAdder().invoke(viewModel.selectedText(), "-") ?: return@item close()
                viewModel.replaceText(newText)
                close()
            }
            item(key = "Task list", label = "To task list") {
                val newText = ListHeadAdder().invoke(viewModel.selectedText(), "- [ ]")
                    ?: return@item close()
                viewModel.replaceText(newText)
                close()
            }
